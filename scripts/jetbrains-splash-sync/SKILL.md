...
8. **README `Path(路径)` quotes can be `"..."` or `` `...` ``.** Both styles are accepted by `JAR_PATH_RE` since the 2026.2 sync. If you hand-edit the README and use yet another delimiter (single quotes, no quotes), the regex silently skips the line and the IDE ends up reported as `WARN no-source`. Stick to double or backtick quotes.
